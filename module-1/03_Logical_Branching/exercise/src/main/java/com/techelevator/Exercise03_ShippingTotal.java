...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        if (weightPounds <= MAX_WEIGHT_POUNDS) {
            ...
            return ((double) MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE);
        }
    }
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        if (!hasDiscount) {
            ...
            if (weightPounds <= MAX_WEIGHT_POUNDS) {
                return ((double) weightPounds * UP_TO_40_LB_RATE) * 0.90;
            } else {
                return (((double) MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE)) * 0.90;
            }
        }
    }
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS) {
            return ((double) weightPounds * UP_TO_40_LB_RATE) * (1 -discountPercentage);
        } else {
            return (((double) MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE) + ((weightPounds - MAX_WEIGHT_POUNDS) * OVER_40_LB_RATE)) *(1 - discountPercentage);
        }
    }
}
