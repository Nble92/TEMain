...
FROM city
JOIN 
	state ON city.state_abbreviation = state.state_abbreviation
WHERE city_name = 'Columbus';
...
SELECT park_name, city_name, park.area
FROM park
JOIN park_state ON park.park_id = park_state.park_id
JOIN state ON state.state_abbreviation = park_state.state_abbreviation
JOIN city ON state.capital = city.city_id;
...
SELECT COUNT(city_name) AS city_count, state_name
FROM city
JOIN state ON city.state_abbreviation = state.state_abbreviation
WHERE census_region = 'Midwest'
GROUP BY state_name;
...
JOIN park ON park_state.park_id = park_state.park_id
GROUP BY state_name;
...
LEFT JOIN park ON park_state.park_id = park_state.park_id
GROUP BY state_name;
...
JOIN genre ON genre.genre.id = movie_genre.genre_id
